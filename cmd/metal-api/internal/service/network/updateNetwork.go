...
func (r *networkResource) updateNetwork(request *restful.Request, response *restful.Response) {
	...
	if len(requestPayload.Prefixes) > 0 {
		...
		if err != nil {
			if service.CheckError(request, response, util.CurrentFuncName(), fmt.Errorf("unable to update Network: %v", err)) {
				return
			}
		}
	...
	err = response.WriteHeaderAndEntity(http.StatusOK, helper.NewNetworkResponse(&newNetwork, usage))
	if err != nil {

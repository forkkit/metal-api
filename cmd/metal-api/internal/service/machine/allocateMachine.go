...
func (r *machineResource) allocateMachine(request *restful.Request, response *restful.Response) {
	...
	if requestPayload.Hostname != nil {
		hostname = requestPayload.Hostname.GetValue()
	}
	...
	sshPubKeys := make([]string, len(requestPayload.SSHPubKeys))
	for i, pubKey := range requestPayload.SSHPubKeys {
	...
	for i, ipAddress := range requestPayload.IPs {
		ips[i] = ipAddress.GetValue()
	}

	networks := make([]v1.MachineAllocationNetwork, len(requestPayload.Networks))
	//for i, nw := range requestPayload.Networks { //TODO
	//	networks[i] = service.FromNetwork(nw)
	//}

	spec := AllocationSpec{
		UUID:        requestPayload.UUID.GetValue(),
		Name:        requestPayload.Common.Name.GetValue(),
		Description: requestPayload.Common.Description.GetValue(),
		Hostname:    hostname,
		ProjectID:   requestPayload.ProjectID,
		PartitionID: requestPayload.PartitionID,
		SizeID:      requestPayload.SizeID,
		Image:       image,
		UserData:    requestPayload.UserData.GetValue(),
		Tags:        tags,
		Networks:    networks,
		IPs:         ips,
		HA:          false,
...
// AllocationSpec is a specification for a machine allocation
...
func Allocate(ds *datastore.RethinkStore, ipamer ipam.IPAMer, allocationSpec *AllocationSpec, mdc mdm.Client) (*metal.Machine, error) {
...
func ValidateAllocationSpec(allocationSpec *AllocationSpec) error {
...
func findMachineCandidate(ds *datastore.RethinkStore, allocationSpec *AllocationSpec) (*metal.Machine, error) {
	var err error
...
func makeNetworks(ds *datastore.RethinkStore, ipamer ipam.IPAMer, allocationSpec *AllocationSpec) (AllocationNetworkMap, error) {
...
func gatherNetworks(ds *datastore.RethinkStore, allocationSpec *AllocationSpec) (AllocationNetworkMap, error) {
...
func GatherNetworksFromSpec(ds *datastore.RethinkStore, allocationSpec *AllocationSpec, partition *metal.Partition, privateSuperNetworks metal.Networks) (AllocationNetworkMap, error) {
	...
	for _, networkSpec := range allocationSpec.Networks {
		...
		if networkSpec.AutoAcquireIP != nil {
			auto = networkSpec.AutoAcquireIP.GetValue()
		}
...
func makeMachineNetwork(ds *datastore.RethinkStore, ipamer ipam.IPAMer, allocationSpec *AllocationSpec, n *AllocationNetwork) (*metal.MachineNetwork, error) {
	if n.Auto {
		ipAddress, ipParentCidr, err := ip.AllocateIP(n.Network, "", ipamer)
		if err != nil {
	...
	for _, ipAddress := range n.IPs {
		...
		err := ds.UpdateIP(&ipAddress, &newIP)
		if err != nil {
			return nil, err
		}
...
func (s AllocationSpec) NoAutoNetworkN() int {
	result := 0
	for _, nw := range s.Networks {
		if nw.AutoAcquireIP != nil && !nw.AutoAcquireIP.GetValue() {
			result++
		}
	}
	return result
...
func (s AllocationSpec) AutoNetworkN() int {

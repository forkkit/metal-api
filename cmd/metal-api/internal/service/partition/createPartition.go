...
func (r *partitionResource) createPartition(request *restful.Request, response *restful.Response) {
	...
	if partition.PrivateNetworkPrefixLength != nil {
		...
		if prefixLength < 16 || prefixLength > 30 {
			if service.CheckError(request, response, util.CurrentFuncName(), fmt.Errorf("private network prefix length is out of range")) {
				return
			}
		}
	}
	...
	err = response.WriteHeaderAndEntity(http.StatusCreated, helper.NewPartitionResponse(p))
	if err != nil {
